...
# --- 設定項目 ---
# 安全のため、RenderのEnvironment（環境変数）からトークンを読み込みます
...
class DistributeView(discord.ui.View):
    ...
    async def get_item(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        try:
            ...
            # ★ご希望の通り、ここに以前あった「実績の記入をお願いします！」の送信処理は完全に削除しました。
...
# コマンド名を「/自販機作成」に変更
@bot.tree.command(name="自販機作成", description="配布パネルを設置・編集します")
@app_commands.default_permissions(administrator=True)
@app_commands.describe(title="パネルのタイトル", description="パネルの説明文（省略可能）", image_url="画像のURL（省略可能）", log_channel="購入通知を送信するチャンネル")
async def setup_panel(interaction: discord.Interaction, title: str, description: str = None, image_url: str = None, log_channel: discord.TextChannel = None):
    await interaction.response.send_message("⚙️ パネルを設置しました！", ephemeral=True)
    
    # 色はご希望通り「緑」で固定（color引数は削除しました）
...
# コマンド名を「/自販機設定」に変更（中身は自販機作成と同じ動きをする予備用として元のコードのまま設置しています）
@bot.tree.command(name="自販機設定", description="配布パネルを設置・編集します")
@app_commands.default_permissions(administrator=True)
@app_commands.describe(title="パネルのタイトル", description="パネルの説明文（省略可能）", image_url="画像のURL（省略可能）", log_channel="購入通知を送信するチャンネル")
async def config_panel(interaction: discord.Interaction, title: str, description: str = None, image_url: str = None, log_channel: discord.TextChannel = None):
    await interaction.response.send_message("⚙️ パネルを設置しました！", ephemeral=True)
    embed_color = discord.Color.green()
    embed = discord.Embed(title=title, color=embed_color)
    if description:
